import { CheckInStatus, PrismaClient, UserRole } from "@prisma/client";
import bcrypt from "bcryptjs";

const prisma = new PrismaClient();

async function main() {
  const passwordHash = await bcrypt.hash("monmate123", 10);

  const testUsers = [
    {
      name: "MonMate Owner",
      email: "owner@monmate.local",
      role: UserRole.OWNER
    },
    {
      name: "MonMate Admin",
  ...
  for (const user of users) {
    console.log(`- ${user.role}: ${user.email} / monmate123`);
  }
